...
def create_plot(sus, inf, rec, des, exp, name):       # Wir erstellen für jede Stadt einen interaktiven Plot
...
def encode(sus, inf, rec, des, exp, name):
    plot_buf = create_plot(sus, inf, rec, des, exp, name)
...
def create_map(sus, inf, rec, des, exp):
    ...
    for city in data:
        folium.Marker(
            ...
            popup=folium.Popup(encode(city['S-population'], city['Infizierte'], city['Genesene'], city['Verstorbene'], city['Exponierte'], city['name']), max_width=300),
            tooltip=city['name']
        ).add_to(m)

    # Karte speichern
    m.save('map_SIR_f.html')
