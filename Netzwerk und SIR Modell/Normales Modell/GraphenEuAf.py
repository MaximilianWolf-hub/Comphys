...
all_suspects = np.array([sus_list])    # enthalten die Populationsenticklungen aller Städte geordnet
...
for i in range(100):
    ...
    infectODEsolverAllEuAf(sus_list, inf_list, rec_list)
    print('Tag:', i+1)
...
# Graphische Darstellung der Populationen in Luanda
index3 = second_column_list.index('LUANDA')
plt.plot(all_suspects[:, index3], label='S-Population')
plt.plot(all_infections[:, index3], label='Infektionen')
plt.plot(all_recovered[:, index3], label='Genesene')
plt.yscale('log')
plt.ylabel('Anzahl an Personen')
plt.xlabel('Tage seit Infektionsbeginn')
plt.legend()
plt.title('Population LUANDA')
plt.savefig('noInfectedFfm.jpeg')
plt.show()
