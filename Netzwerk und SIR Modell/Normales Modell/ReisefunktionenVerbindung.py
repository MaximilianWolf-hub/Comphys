...
# Daten wird alle Verbindungen enthalten, hierbei ist in der ersten Spalte die jeweilige Stadt vermerkt und in
# den restlichen Spalten einer Zeile die jeweils möglichen Reiseziele
data = []

# Reisewahrscheinlichkeit
with open('allConnectionsCombined.txt', 'r', encoding='utf-8') as f:  # Daten sind mit Komma getrennt
    ...
    for i in range(len(pop_list)):  # iteriert über alle Einträge in der Pop-Liste
        ...
        for j in range(1, len(data[i][:])):  # Starte bei 1, da data[i][0] die aktuelle Stadt ist
            ...
            if city_name in cities_combined:
                ...
                pops += population_combined[index]
